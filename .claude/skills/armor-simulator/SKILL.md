...
│   └── detection_noise.hpp            DetectionNoise (addPixelNoise/shouldDetect)
├── src/
...
pose = {position, orientation}           # PnP 解算的 3D 位姿 (与像素一致)
yaw                                     # 从 PnP 提取的朝向 (rad)
area                                    # 像素面积 (px²)
```
...
**噪声量级** (σ_min=1.5, d_opt=5m, k=0.125): 2m→2.6px, 3m→2.0px, 5m→1.5px(最优), 8m→2.6px, 12m→7.6px
...
// 局部坐标: {(-w/2,0,-h/2), (-w/2,0,h/2), (w/2,0,h/2), (w/2,0,-h/2)}
...
/armor_simulation_node:
  ros__parameters:
    ...
    # 像素噪声 (U 形距离模型)
    pixel_noise_optimal: 1.5; pixel_noise_optimal_distance: 5.0; pixel_noise_curvature: 0.125
    use_outliers: true; outlier_probability: 0.05; outlier_std: 10.0

    # 检测概率
    detection_probability: 0.95; miss_probability: 0.05
